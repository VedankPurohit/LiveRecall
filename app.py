...
with tab1:
    ...
    #                 # --- NEW: SLIDER + PREVIEW GALLERY ---
    #                 # Keep track of which image index is selected
    #                 if 'selected_image_index' not in st.session_state:
    #                     st.session_state.selected_image_index = 0

    #                 # Slider to pick which image to show
    #                 st.session_state.selected_image_index = st.slider(
    #                     "Slide through images",
    #                     0,
    #                     len(image_locations) - 1,
    #                     st.session_state.selected_image_index
    #                 )

    #                 # Show the main image
    #                 main_image = image_locations[st.session_state.selected_image_index]
    #                 st.image(
    #                     main_image,
    #                     caption=main_image.replace("Temp\\screenshot_","")
    #                                      .replace(".png","")
    #                                      .replace(".jpg",""),
    #                     use_column_width=True
    #                 )

    #                 # Show a horizontal preview row below
    #                 import base64, os
    #                 st.markdown("<h4 style='text-align: center;'>Preview</h4>", unsafe_allow_html=True)
    #                 preview_html = '<div style="display: flex; flex-wrap: nowrap; overflow-x: auto;">'
    #                 for i, img_path in enumerate(image_locations):
    #                     with open(img_path, "rb") as f:
    #                         data = f.read()
    #                     b64 = base64.b64encode(data).decode()

    #                     # Highlight the currently selected image
    #                     border_color = "3px solid red" if i == st.session_state.selected_image_index else "3px solid transparent"
    #                     caption = os.path.basename(img_path).replace("screenshot_","").replace(".png","").replace(".jpg","")

    #                     preview_html += f"""
    #                         <div style="text-align:center; margin-right:10px;">
    #                             <img src="data:image/png;base64,{b64}"
    #                                  style="width:100px; margin:5px; border:{border_color};" />
    ...
    #             st.warning("Please enter a search term.")
    #     except:
    #         st.warning("No images found for your search term. Or some Error occurred.")
    ...
    #     try:
    #         if search_term:
    #             # Retrieve and store images in session_state so we can browse them
    #             image_locations = get_image(search_term)
    #             st.session_state.prev_search_term = search_term  # keep track of what was searched

    #             if image_locations:
    #                 st.success(f"Found {len(image_locations)} images!")
                    
    #                 # --- BROWSING CONTROLS --- 
    #                 # 1) Set up session state for which image is displayed
    #                 if "current_image_idx" not in st.session_state:
    #                     st.session_state.current_image_idx = 0

    #                 # If the new search found images, reset to 0
    #                 st.session_state.current_image_idx = 0  
    #                 image_count = len(image_locations)

    #                 # 2) Lay out next/previous buttons and slider in columns
    #                 col_prev, col_slider, col_next = st.columns([1, 4, 1], gap="large")
    ...
    #                 with col_slider:
    #                     st.session_state.current_image_idx = st.slider(
    #                         "Image Index",
    #                         min_value=0,
    #                         max_value=image_count - 1,
    ...
    #                 with col_next:
    #                     if st.button("Next →"):
    #                         if st.session_state.current_image_idx < image_count - 1:
    #                             st.session_state.current_image_idx += 1

    #                 # 3) Show the main large image
    #                 current_idx = st.session_state.current_image_idx
    #                 st.image(
    #                     image_locations[current_idx],
    #                     caption=f"Image {current_idx + 1} of {image_count}",
    #                     use_column_width=True
    #                 )

    #                 # 4) Show thumbnail previews below
    #                 st.markdown("---")
    #                 st.markdown("### Preview All Images")
                    
    #                 # Create columns for each thumbnail
    #                 thumb_cols = st.columns(image_count, gap="small")
    #                 for i, img_path in enumerate(image_locations):
    #                     # Each column gets a button + thumbnail
    #                     with thumb_cols[i]:
    #                         # Use a button to let user select this image
    #                         if st.button(f"View {i+1}", key=f"view_btn_{i}"):
    #                             st.session_state.current_image_idx = i
                            
    #                         # Display the thumbnail
    #                         # `width=100` to keep them smaller; adjust as needed
    #                         st.image(img_path, width=100)

    #                 # OPTIONAL: If you really want to remove the images, do it once the user is done:
    #                 # st.button("Clear Images", on_click=RemoveImages)
    ...
    #             st.warning("Please enter a search term.")
    #     except:
    #         st.warning("No images found or an error occurred.")
